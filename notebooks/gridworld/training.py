...
def train_td(build, batches, feats, chunks, kind, key, alpha=0.05, tau=0.01, lr=3e-4):
    ...
    F, C = jnp.asarray(feats), jnp.asarray(chunks)

    def next_value(tp, ns, gf):
    ...
    def step(carry, b):
        ...
        tq = jax.lax.stop_gradient(
            b["reward"] + b["discount"] * next_value(target, b["next_state"], gf))
...
def train_contrastive(build, batches, feats, key, lr=3e-4, logsumexp_coeff=0.1,
    ...
    def step(carry, b):
        ...
        def loss(p):
            # For CRL every (s, a) is scored against every goal in the batch; for CARL every
            # (s, g) against every action sequence. The diagonal is the pair that occurred.
            lg = logits(p, F[b["state"]], F[b["goal"]], b["seq"])
            eye = jnp.eye(lg.shape[0])
            if loss_fn == "binary_nce":
                # Balanced accuracy: a batch is 255/256 negatives, so plain accuracy is ~1.0 for a
                # critic that simply calls everything a negative. Average the two classes instead.
                pos = jnp.sum((lg > 0) * eye) / jnp.sum(eye)
                neg = jnp.sum((lg <= 0) * (1 - eye)) / jnp.sum(1 - eye)
                acc = 0.5 * (pos + neg)
                return jnp.mean(eye * jax.nn.softplus(-lg) + (1 - eye) * jax.nn.softplus(lg)), acc
            # accuracy: does the true pair win its row? (carl_crl calls this categorical_accuracy)
