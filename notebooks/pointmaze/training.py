...
def train_td(build, batches, cand, kind, key, alpha=0.05, tau=0.01, lr=3e-4):
    def make():
        ...
        def next_value(tp, ns, g, C):
            ...
            #
            # The soft value over a continuous action space is alpha * log integral exp(Q/alpha)
            # da. Estimating that from n_cand samples needs the - log(n_cand) term: without it
            # the bonus grows with the pool size, and worse, with how flat Q is across
            # candidates -- largest far from the goal, so it tilts the policy away from it.
            if kind == "sac":
        ...
        def run(key, batches, C):
            ...
            def step(carry, b):
                ...
                tq = jax.lax.stop_gradient(
                    b["reward"] + b["discount"] * next_value(target, b["next_state"], b["goal"], C))
                ...
                def loss(p):
                    ...
                    if loss_fn == "binary_nce":
                        ...
                        return jnp.mean(eye * jax.nn.softplus(-lg)
                                        + (1 - eye) * jax.nn.softplus(lg)), 0.5 * (pos + neg)
